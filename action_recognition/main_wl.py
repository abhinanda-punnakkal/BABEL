...
class Processor():
        ...
        # Code adapted form https://github.com/vandit15/Class-balanced-loss-pytorch
        def CB_loss(self, labels, logits, samples_per_cls, no_of_classes, loss_type, beta, gamma= 0.999):
                ...
                labels_one_hot = F.one_hot(labels, no_of_classes).float().cuda(self.output_device)

                weights = torch.tensor(weights).float().cuda(self.output_device)
        ...
        def train(self, epoch, wb_dict, save_model=False):
                ...
                for batch_idx, (data, label, sid, seg_id, chunk_n, anntr_id, index) in enumerate(process):
                        ...
                        loss = l1+ self.CB_loss(label, output,
                                                                                self.samples_per_class,
                                                                                nb_classes, self.loss_type,
                                                                                self.arg.beta, self.arg.gamma)
                        # backward
        ...
        def eval(self, epoch,
                ...
                for ln in loader_name:
                        ...
                        if save_score:
                                ...
                                with open('{}/epoch{}_{}_per_class_acc.pkl'.format(
                                                self.arg.work_dir, epoch + 1, ln), 'wb') as f:
                                        pickle.dump(per_class_acc, f)
